...
function where(){//funcão para selecionar apenas uma linha das formações
    ...
    xhttp.onreadystatechange = function(){
        if(this.readyState === 4 && this.status === 200){
            ...
            form.id = "descriptionCard"
            form.innerText = resposta[0].FORMACAO;

            var inicio = document.createElement('div');
            inicio.id = "descriptionCard"
            inicio.innerText = resposta[0].DataInicioFormacao;

            var fim = document.createElement('div');
            fim.id = "descriptionCard"
            fim.innerText = resposta[0].DataFinalFormacao;


            var inst = document.createElement('div');
            inst.id = "descriptionCard"
            inst.innerText = resposta[0].Instituicao;

            var list = [form, inst, inicio, fim]

            for (var i = 0; i < list.length; i++){
                document.getElementById("Formacao").append(list[i])
                console.log[list[i]]
            }
        }
    }
...
function update(){//função para update de formação, por algum motivo não está funcionando corretamente na data e hora
    var id = document.getElementById("ID_FORMACAO").value;
    var formacao = document.getElementById("formacao").value;
...
//gerando o curriculo a partir de um fetch
